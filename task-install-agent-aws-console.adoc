...
summary: È possibile distribuire un agente Console in AWS direttamente dalla NetApp Console. Prima di distribuire un agente Console in AWS dalla Console, è necessario configurare la rete e preparare le autorizzazioni AWS. 
---
= Distribuisci un agente Console in AWS dalla NetApp Console
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
È possibile distribuire un agente Console in AWS direttamente dalla NetApp Console. Prima di distribuire un agente Console in AWS dalla Console, è necessario configurare la rete e preparare le autorizzazioni AWS.
...
Crea l'agente della Console direttamente dalla Console basata sul web.
...
. Nella pagina *Panoramica*, selezionare *Distribuisci agente > AWS*.
. Per creare l'agente Console, seguire i passaggi della procedura guidata:
. Nella pagina *Introduzione*, rivedi la panoramica del processo.
. Nella pagina *Credenziali AWS*, specifica la tua regione AWS e poi scegli un metodo di autenticazione, che può essere un ruolo IAM che la Console può assumere oppure una chiave di accesso e una chiave segreta AWS.
+
...
. Nella pagina *Rete*, specificare una VPC, una subnet e una coppia di chiavi per l'agente, scegliere se abilitare un indirizzo IP pubblico e, facoltativamente, specificare una configurazione proxy.
+
