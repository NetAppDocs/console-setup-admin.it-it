...
summary: 'Se sei un utente VMware, puoi utilizzare un file OVA per installare un agente della Console nel tuo vCenter. Il download del file OVA o l"URL sono disponibili tramite la NetApp Console.' 
---
= Installa un agente NetApp Console in locale usando vCenter
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Se sei un utente VMware, puoi utilizzare un file OVA per distribuire un agente della Console nel tuo vCenter. Il download del file OVA o l'URL sono disponibili tramite la NetApp Console.


NOTE: Quando si distribuisce un agente Console con i propri vCenter strumenti, è possibile utilizzare la console web della macchina virtuale per eseguire attività di manutenzione. link:task-agent-vm-config.html["Scopri di più sulla console VM per l'agente."]
...
NOTE: Non è possibile gestire le risorse in Google Cloud con un agente Console installato in locale. Per gestire le risorse in Google Cloud, installare un agente in Google Cloud.
...
.Risultato
Ora dovresti disporre di chiavi di accesso utente IAM con le autorizzazioni necessarie. Dopo aver installato l'agente della Console, associa queste credenziali all'agente della Console dalla Console.
...
NetApp supporta l'installazione dell'agente Console nel tuo ambiente vCenter. Il file OVA include un'immagine VM preconfigurata che puoi distribuire nel tuo ambiente VMware. Un download del file o la distribuzione tramite URL è disponibile direttamente dalla NetApp Console. Include il software dell'agente Console e un certificato autofirmato.
...
=== Distribuisci l'agente nel tuo vCenter environment

Accedi al tuo ambiente vCenter per distribuire l'agente.

.Passi
. Se il tuo ambiente lo richiede, carica il certificato autofirmato nell'elenco dei certificati attendibili. Sostituisci questo certificato dopo l'installazione.
...
. Nella finestra *Personalizza modello*, compilare i seguenti campi:
+
** *Informazioni proxy*
+
*** Se hai selezionato proxy esplicito, inserisci il nome host o l'indirizzo IP del proxy server e il numero di porta, nonché il nome utente e la password.
...
+
**** NetApp consiglia di lasciare questa casella deselezionata in modo che l'installazione includa un controllo di configurazione dell'agente. Il controllo di configurazione verifica che l'agente abbia accesso di rete agli endpoint richiesti. Se la distribuzione non riesce a causa di problemi di connettività, è possibile accedere al report di convalida e ai log dall'host dell'agente. In alcuni casi, se si è certi che l'agente abbia accesso di rete, è possibile scegliere di saltare il controllo. Ad esempio, se si utilizza ancora la link:reference-networking-saas-console-previous.html["punti finali precedenti"] utilizzata per gli aggiornamenti dell'agente, la convalida non riesce e restituisce un errore. Per evitare ciò, selezionare la check box per installare senza un controllo di convalida. link:reference-networking-saas-console-previous.html#update-endpoint-list["Scopri come aggiornare l'elenco degli endpoint"].
...
*** *Domini di ricerca*: Specifica il domain name da utilizzare per la risoluzione del nome host. Ad esempio, se il FQDN è console10.searchdomain.company.com, inserisci searchdomain.company.com.
...
. Selezionare *Avanti*.
. Rivedi i dettagli nella finestra *Ready to complete*, quindi seleziona *Finish*.
+
