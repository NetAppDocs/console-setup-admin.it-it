...
summary: 'Scopri le novità relative all"amministrazione della NetApp Console.' 
---
= Cosa c'è di nuovo in NetApp Console
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
Quando usi BlueXP in modalità limitata o privata, l'interfaccia utente è accessibile dalla macchina virtuale Connector distribuita nella tua area cloud o on-premises. Per impostazione predefinita, BlueXP utilizza un certificato SSL autofirmato per fornire accesso HTTPS sicuro dal browser web locale.
...
Abbiamo aggiornato le policy AWS e Azure per il Connector per rimuovere le autorizzazioni non più necessarie. Le autorizzazioni erano relative a BlueXP edge caching e al rilevamento e alla gestione dei cluster Kubernetes in Azure e AWS.
...
----
Queste autorizzazioni sono necessarie per il supporto dei Virtual Machine Scale Sets in Cloud Volumes ONTAP. Se hai già dei Connector e vuoi usare questa nuova funzionalità, dovrai aggiungere queste autorizzazioni alle policy personalizzate associate alle credenziali di Azure.
...
Se hai un agente esistente in esecuzione su RHEL 7 o CentOS 7, NetApp non supporta l'aggiornamento o la conversione del sistema operativo a RHEL 8 o 9. Devi creare un nuovo agente su un sistema operativo supportato. Segui questi passaggi high-level per effettuare la transizione:
...
** https://docs.netapp.com/us-en/storage-management-fsx-ontap/use/task-create-fsx-system.html["Creare o scoprire un sistema FSx per ONTAP"^]
