...
Quando si distribuisce un agente Console dalla Console, è necessario fornire l'ARN di un ruolo IAM o le chiavi di accesso per un utente IAM. Il metodo di autenticazione deve disporre delle autorizzazioni per distribuire la console in AWS. Le autorizzazioni richieste sono elencate nel collegamento:task-install-agent-aws-the Console.html#aws-permissions-agent[Criterio di distribuzione dell'agente per AWS].
