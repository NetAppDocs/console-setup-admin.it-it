...
summary: Aggiungi e gestisci le credenziali AWS in modo che la NetApp Console disponga delle autorizzazioni necessarie per distribuire e gestire le risorse cloud nei tuoi account AWS.  Se gestisci più abbonamenti AWS, puoi assegnare a ciascuno di essi credenziali AWS diverse dalla pagina Credenziali. 
---
...
[role="lead"]
Aggiungi e gestisci le credenziali AWS per poter distribuire e gestire le risorse cloud nei tuoi account AWS dalla NetApp Console. Se gestisci più abbonamenti AWS Marketplace, puoi assegnare a ciascuno di essi credenziali AWS diverse dalla pagina Credenziali.
...
+
L'aggiunta di nuove credenziali AWS alla Console fornisce le autorizzazioni necessarie per creare un agente della Console. <<add-aws-credentials-agent-creation,Scopri come aggiungere le credenziali AWS alla NetApp Console>> .
...
La NetApp Console consente di fornire le credenziali AWS in diversi modi: un ruolo IAM associato all'istanza dell'agente, assumendo un ruolo IAM in un account attendibile o fornendo le chiavi di accesso AWS. link:concept-accounts-aws.html["Scopri di più sulle credenziali e le autorizzazioni AWS"] .
...
link:concept-accounts-aws.html["Scopri come la NetApp Console utilizza le credenziali e le autorizzazioni AWS"].
...
NOTE: Se hai distribuito un agente della Console dalla Console, sono state aggiunte automaticamente le credenziali AWS per l'account in cui hai distribuito un agente della Console. In questo modo si garantisce che siano disponibili le autorizzazioni necessarie per la gestione delle risorse.
...
Se si desidera fornire alla Console le chiavi AWS per un utente IAM, è necessario concedere a tale utente le autorizzazioni richieste. La policy IAM della Console definisce le azioni e le risorse AWS che la Console è autorizzata a utilizzare.
...
NOTE: Potrebbero volerci alcuni minuti prima che le nuove credenziali del tuo provider cloud diventino disponibili.
...
Configura un ruolo IAM che consenta all'applicazione software as a service (SaaS) NetApp Console di assumere il ruolo.
...
.Risultato
Il ruolo IAM ora dispone delle autorizzazioni necessarie. <<add-the-credentials,Ora puoi aggiungerlo alla Console>>.
...
.Prima di iniziare
Se hai appena creato il ruolo IAM, potrebbero essere necessari alcuni minuti prima che sia disponibile per l'uso. Attendi qualche minuto prima di aggiungere le credenziali alla Console.
...
. Nella pagina *Credenziali dell'organizzazione*, seleziona *Aggiungi credenziali* e segui i passaggi della procedura guidata.
+
.. *Posizione delle credenziali*: seleziona *Amazon Web Services > Console*.
...
Per i dettagli, fare riferimento a https://docs.netapp.com/us-en/storage-management-fsx-ontap/requirements/task-setting-up-permissions-fsx.html["Documentazione della console per Amazon FSx per ONTAP"^]
...
.Prima di iniziare
È necessario creare un agente Console prima di poter configurare un abbonamento. link:concept-agent-install-options.html["Scopri come creare un agente Console"].
...
Quando si sottoscrive un abbonamento tramite AWS Marketplace, l'ultimo passaggio consiste nell'associare l'abbonamento alla propria organizzazione. Se non si completa questo passaggio, non sarà possibile utilizzare l'abbonamento con l'organizzazione.
...
.Passi
. Verifica di non aver associato il tuo abbonamento all'organizzazione della tua Console.
+
.. Dal menu di navigazione, seleziona *Amministrazione > Licenses and subscriptions*.
...
.Passi
. Selezionare *Amministrazione > Credenziali*.
. Nella pagina *Credenziali dell'organizzazione*, seleziona il menu azioni per un set di credenziali, quindi seleziona *Modifica credenziali*.
...
TIP: Non è possibile eliminare le credenziali per un profilo di istanza associato a un agente Console.
