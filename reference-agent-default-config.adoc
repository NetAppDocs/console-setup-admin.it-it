...
summary: 'Scopri di più sulle configurazioni predefinite dell"agente della console per distribuzioni standard (con accesso a Internet) su AWS, Azure e Google Cloud, nonché sulle distribuzioni limitate (senza accesso a Internet) per ambienti on-premise.' 
---
= Configurazione predefinita dell'agente della console in NetApp Console
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* Il  `occm`servizio dipende dal servizio MySQL.
+
Se il servizio MySQL non è attivo, allora anche il  `occm` servizio non è attivo.
