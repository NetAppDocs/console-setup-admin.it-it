...
---
= Distribuisci l'agente della NetApp Console in modalità limitata
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
+
link:task-prepare-restricted-mode.html#step-3-review-host-requirements["Requisiti dell'agente di revisione"].
...
+
Se invece si utilizza un indirizzo IP SKU standard, la Console utilizza l'indirizzo IP _privato_ dell'agente della Console, anziché l'IP pubblico. Se il computer utilizzato per accedere alla Console non riesce a raggiungere l'indirizzo IP privato, la Console non funziona.
...
+
Un'identità gestita consente alla macchina virtuale dell'agente della console di identificarsi con l'ID Microsoft Entra senza credenziali. https://docs.microsoft.com/en-us/azure/active-directory/managed-identities-azure-resources/overview["Scopri di più sulle identità gestite per le risorse di Azure"^] .
...
--
Fornisci alla NetApp Console le credenziali dell'entità servizio di Azure che hai configurato in precedenza.
