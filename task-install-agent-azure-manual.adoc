...
---
= Installa manualmente l'agente NetApp Console in Azure
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
La distribuzione di un agente in Azure avvia una VM con Linux e il software dell'agente Console in una VNet a scelta.


NOTE: Distribuisci un agente Console nella stessa area di Azure dei sistemi Cloud Volumes ONTAP che gestisce, oppure nella  https://docs.microsoft.com/en-us/azure/availability-zones/cross-region-replication-azure#azure-cross-region-replication-pairings-for-all-geographies["Coppia di regioni di Azure"^]. Ciò garantisce che venga utilizzata una connessione Azure Private Link tra Cloud Volumes ONTAP e gli account di storage associati.

.Prima di iniziare
* link:concept-agent-install-options.html#agent-cloud-manual-deploy["Scopri quando eseguire la distribuzione manuale in Azure"].
