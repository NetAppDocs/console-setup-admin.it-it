...
---
= Autorizzazioni di Google Cloud per l'agente della NetApp Console
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
| config.deployments.get | NO | SÌ | SÌ 
...
| calcola.dischi.crea .8+| Per creare e gestire dischi sui sistemi Cloud Volumes ONTAP. | SÌ | SÌ | NO 
...
| compute.forwardingRules.update | Aggiorna le regole di inoltro esistenti per la gestione del traffico. | NO | SÌ | NO 
...
| registrazione.logEntries.create .2+| Creare e instradare voci di registro per il monitoraggio, il debug e la verifica. | SÌ | SÌ | NO 
...
| cloudbuild.workerpools.get | Per accedere alle informazioni del pool di worker durante la distribuzione in modalità privata e la conversione dei sistemi Cloud Volumes ONTAP utilizzando Infrastructure Manager. | SÌ | SÌ | SÌ 


| cloudbuild.workerpools.list | Per elencare le informazioni sul pool di worker durante la distribuzione in modalità privata dei sistemi Cloud Volumes ONTAP utilizzando Infrastructure Manager. | SÌ | NO | NO 
...
L'autorizzazione  `compute.disks.update` è stata aggiunta per aggiornare i parametri IOPS e throughput su un aggregate o disco Hyperdisk Balanced nei sistemi Cloud Volumes ONTAP.
...
Le autorizzazioni  `cloudbuild.workerpools.get` e  `cloudbuild.workerpools.list` sono state aggiunte per supportare Infrastructure Manager nelle distribuzioni in modalità privata di Cloud Volumes ONTAP in Google Cloud.
...
L' `compute.forwardingRules.update`autorizzazione è stata aggiunta per supportare Infrastructure Manager nelle distribuzioni Cloud Volumes ONTAP in Google Cloud.
