...
summary: 'Per creare un agente Console in Google Cloud dalla NetApp Console o utilizzando Google Cloud, è necessario configurare la rete, preparare le autorizzazioni di Google Cloud, abilitare le API di Google Cloud e quindi creare l"agente Console.' 
---
= Distribuisci un agente Console in Google Cloud da NetApp Console
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
È possibile distribuire un agente Console in Google Cloud dalla Console. È necessario configurare la rete, preparare le autorizzazioni di Google Cloud, abilitare le API di Google Cloud e quindi distribuire l'agente Console.
...
.Prima di iniziare
* link:concept-agent-install-options.html#agent-console-deploy["Scopri quando effettuare il deployment dalla Console"].
...
Endpoint contattati dalla console NetApp:: Quando si utilizza l'applicazione SaaS Console NetApp basata sul web, questa si connette a diversi endpoint per completare le attività di gestione dei dati. Ciò include gli endpoint contattati per distribuire l'agente Console dalla Console stessa.
+
--
...
NOTE: Quando distribuisci un agente in Google Cloud, l'agente crea un bucket in cui archiviare i file di distribuzione.
...
* Una VPC e una subnet che soddisfino i requisiti di rete.
...
** *Tag di rete*: Aggiungi un tag di rete all'istanza dell'agente Console se utilizzi un proxy trasparente. I tag di rete devono iniziare con una lettera minuscola e possono contenere lettere minuscole, numeri e trattini. I tag devono terminare con una lettera minuscola o un numero. Ad esempio, potresti usare il tag `console-agent-proxy`.
** *Criteri del firewall*: Scegliere se creare un nuovo criterio del firewall o se selezionare un criterio del firewall esistente che consenta le regole inbound e outbound richieste.
+
...
Se disponi di bucket di Google Cloud Storage nello stesso account Google Cloud in cui hai creato l'agente della Console, un sistema Google Cloud Storage viene visualizzato automaticamente nella pagina *Sistemi* https://docs.netapp.com/us-en/storage-management-google-cloud-storage/index.html["Scopri come gestire Google Cloud Storage dalla Console"^].
