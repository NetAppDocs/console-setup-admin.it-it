...
Puoi aggiungere due tipi di membri alla tua organizzazione: un account utente e un account di servizio. Le applicazioni utilizzano account di servizio per eseguire automaticamente le attività API.  Le persone utilizzano gli account utente per accedere e gestire le risorse.
...
Quando si rimuove un membro, il sistema revoca le sue autorizzazioni ma conserva i suoi account Console e NetApp Support Site.
...
. Seleziona *Ricrea*.
. Scarica o copia l'ID client e il segreto client. + La console visualizza il segreto del client solo una volta. Copialo o scaricalo e conservalo in modo sicuro.
...
Gli utenti devono configurare nuovamente l'MFA quando effettuano l'accesso dopo la rimozione. Se l'utente ha perso l'accesso al proprio dispositivo MFA solo temporaneamente, può utilizzare il codice di ripristino salvato durante la configurazione di MFA per effettuare l'accesso.
