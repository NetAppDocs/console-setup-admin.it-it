...
---
= Distribuisci un agente Console in Azure dalla NetApp Console
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Per distribuire un agente Console in Azure dalla NetApp Console, è necessario configurare la rete, preparare le autorizzazioni di Azure e quindi distribuire l'agente Console.
...
. Nella pagina *Panoramica*, selezionare *Distribuisci agente > Azure*.
...
. Nella pagina *Autenticazione della macchina virtuale*, seleziona l'opzione di autenticazione che corrisponde alla configurazione delle autorizzazioni di Azure:
+
...
. Nella pagina *Dettagli*, inserisci un nome per l'agente, specifica i tag e scegli se desideri che la Console crei un nuovo ruolo con le autorizzazioni richieste oppure se desideri selezionare un ruolo esistente che hai configurato con link:reference-permissions-azure.html["i permessi richiesti"].
+
...
Se si dispone di Azure Blob Storage nello stesso account Azure in cui è stato creato l'agente Console, Azure Blob Storage verrà visualizzato automaticamente nella pagina *Sistemi*. https://docs.netapp.com/us-en/bluexp-blob-storage/index.html["Scopri come gestire l'archiviazione BLOB di Azure dalla NetApp Console"^]
