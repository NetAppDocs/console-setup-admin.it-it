...
=== Autorizzazioni aggiuntive di backup e ripristino
...
L'agente effettua le seguenti richieste API quando si utilizza la funzionalità Cerca e ripristina. Queste autorizzazioni sono necessarie solo se hai abilitato le funzionalità di indicizzazione legacy prima del rilascio dell'indicizzazione v2:
...
Le seguenti autorizzazioni sono state rimosse dall'esempio del ruolo JSON in modo che l'esempio possa riflettere le autorizzazioni minime richieste per Backup e Ripristino:
...
====
Sono state aggiunte le seguenti autorizzazioni all'esempio di ruolo JSON:
