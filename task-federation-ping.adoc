...
summary: 'Federati con il tuo provider IdP PingFederate per abilitare l"accesso singolo (SSO) per la NetApp Console.  Ciò consente agli utenti di accedere utilizzando le proprie credenziali aziendali.' 
---
...
È possibile configurare una connessione federata con PingFederate per abilitare il single sign-on (SSO) per la Console. Il processo prevede la configurazione del server PingFederate affinché consideri la Console come un service provider e la successiva creazione della connessione nella Console.
...
+
.. Utilizzare i seguenti valori quando si configura PingFederate per considerare attendibile la NetApp Console:
+
...
*** Per *Audience/Entity ID*, utilizzare  `urn:auth0:netapp-cloud-account:<fed-domain-name-pingfederate>` dove <fed-domain-name-pingfederate> è il domain name per la federazione. Ad esempio, se il dominio è  `example.com`, l'Audience/Entity ID sarà  `urn:auth0:netapp-cloud-account:fed-example-com-pingfederate`.
