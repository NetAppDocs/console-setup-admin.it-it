...
---
= Distribuisci un agente Console dall'AWS Marketplace
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
È possibile distribuire un agente Console in AWS direttamente da AWS Marketplace. Per distribuire un agente Console da AWS Marketplace, è necessario configurare la rete, preparare le autorizzazioni AWS, verificare i requisiti dell'istanza e quindi distribuire l'agente Console.
...
.Prima di iniziare
* link:concept-agent-install-options.html#agent-marketplace-deploy["Scopri quando effettuare il deployment da AWS Marketplace"].
...
La creazione dell'agente Console da AWS Marketplace distribuisce un'istanza EC2 in AWS utilizzando una configurazione predefinita. link:reference-agent-default-config.html["Scopri la configurazione predefinita per l'agente Console"] .
...
Apri un browser web e vai su https://console.netapp.com["NetApp Console"^] per iniziare a utilizzare l'agente Console con la Console.
