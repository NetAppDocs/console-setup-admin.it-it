...
---
= Configura un agente NetApp Console per usare un proxy server
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
IMPORTANT: Una volta configurato un proxy, non è possibile modificarne il tipo di proxy. Se è necessario cambiare il tipo di proxy, bisogna rimuovere l'agente Console e aggiungerne uno nuovo con il nuovo tipo di proxy.
...
Se hai configurato un agente Console per utilizzare un proxy server, puoi abilitare il traffico API diretto sull'agente Console per inviare le chiamate API direttamente ai servizi del cloud provider senza passare attraverso il proxy. Gli agenti in esecuzione su AWS, Azure o Google Cloud supportano questa opzione.
...
https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-enabling-private-link.html["Scopri di più sull'utilizzo di un collegamento privato di Azure o di endpoint di servizio con Cloud Volumes ONTAP"^]
