...
---
= Associa le credenziali dell'agente della console al tuo abbonamento a Google Cloud
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Quando crei un agente Console in Google Cloud, puoi utilizzare le credenziali associate per abbonarti a NetApp Intelligent Services, che includono Cloud Volumes ONTAP e i servizi dati NetApp .  È necessario associare l'abbonamento all'agente le cui credenziali si desidera utilizzare per pagare i servizi.
...
La sostituzione dell'attuale abbonamento al marketplace con uno nuovo aggiorna l'abbonamento per i sistemi Cloud Volumes ONTAP esistenti e nuovi.
...
L'abbonamento ai servizi dati NetApp tramite Google Cloud Marketplace potrebbe non riuscire a causa di autorizzazioni errate o di un mancato reindirizzamento alla Console. In tal caso, attenersi alla seguente procedura per completare la procedura di abbonamento.
